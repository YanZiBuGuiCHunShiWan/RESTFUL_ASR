...
def read_wave(path):
    ...
    with contextlib.closing(wave.open(path, 'rb')) as wf:
        ...
        assert sample_rate in (8000,16000, 32000,44100, 48000),"Sampling rate error,sampling rate should in [16000,32000,44100,48000] ."
        nframes=wf.getnframes()
        pcm_data = wf.readframes(nframes)
        duration_time=nframes/float(sample_rate)
        # 多通道通道转单通道
        if num_channels != 1:
            data = []
        ...
        return pcm_data, sample_rate,duration_time   #该函数讲多通道度语音转化维拉单通道
...
def crop_audio_vad(audio_path, aggressiveness=1, frame_duration_ms=30):
    audio, sample_rate,duration_time = read_wave(audio_path)
    vad = webrtcvad.Vad(aggressiveness)
    frames = frame_generator(frame_duration_ms, audio, sample_rate)
    frames = list(frames)
    audios_bytes = vad_collector(sample_rate, frame_duration_ms, 300, vad, frames)
    return audios_bytes
